    ...
    <div class="admin-form">
        ...
        <div class="admin-send" v-if="canModify">
            <template v-if="!check && !view" >
                ...
                <el-button v-if="showReturn" type="primary" :disabled="disableBtn" @click="returnback">返回生产节点智能统计</el-button>
            </template>
...
export default {
    ...
    mounted () {
        ...
        if(this.intel){
            ...
            let quantity = localStorage.getItem("num")
            let building = ''
            let eartagFile=''
            let period = localStorage.getItem("period")
            if(store !== null){
                store.forEach((item) => {
                building += item.col +"栏/"+item.building + "栋;" 
                item.earTags.forEach((i) => {
                    eartagFile += i.earTag + ","
                    })
                })
                eartagFile = eartagFile.substring(0 , eartagFile.lastIndexOf(","))
                this.models = {quantity , building , eartagFile , period}
