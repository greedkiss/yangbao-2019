...
<script>
  import { getIntelTable , getUserById ,getStageDetail ,getStageTable ,getSheepBuilding ,getSheepCol ,moveSheep ,moveSheepAll ,updateSheepAllMe ,getDouble} from '@/util/getdata'
  export default{
    props:{
      sumtext:{
        type: String
    ...
    methods:{
      ...
      fetchData(){
        ...
        getIntelTable(this.user.userFactory , stage ,2).then(res => {
        ...
        getDouble(this.user.userFactory , stage , 1).then(res=>{
            let list = res.data.lists
          list.forEach((item) => {
            let {building , col , number ,trademark , id , immune} = item
            let buildingNu = building + "栋"
            ...
            this.earTagList.set(earTagNu , trademark)
            this.tableData3.push({buildingNu , columnNu , earTagNu ,totalNu ,id ,immune})
          })
        })
