<template>
  <div class="admin-form" v-loading="loading">
    ...
    <div class="choose-radio last-radio">
      <div class="radio-title">是否一次性打印成品二维码：</div>
      <template>
        <div class="radio">
    ...
    <template>
      <el-table :data="tableData" style="width: 100%">
        <el-table-column label="成品种类" prop="dishesType" width="150"></el-table-column>
        <el-table-column label="成品名称" prop="dishesName" width="150"></el-table-column>
        <el-table-column label="羊肉用量（克/份）" width="150">
          <template slot-scope="scope">
            <span>{{scope.row.mutton*1000}}</span>
          </template>
        </el-table-column>
        <el-table-column label="默认图片" prop="name" width="150">
          <template slot-scope="scope">
            <el-button type="text" @click="openDialog(scope.$index, scope.row)">查看</el-button>
          </template>
        </el-table-column>
        <el-table-column label="设为本店默认出品" prop="isEnable" width="150">
          <template slot-scope="scope">
            <span v-if="scope.row.isEnable == 0">否</span>
            <span v-else>是</span>
          </template>
        </el-table-column>
        <el-table-column label="批量打印" prop="isDisposable" width="150">
          <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      window
        .fetch(baseUrl + "/dishes/addDishes", {
          method: "POST",
          headers,
          body: form
        })
