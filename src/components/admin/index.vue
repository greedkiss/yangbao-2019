<template>
  <div class="app-home">
    <admin-head
      :username="user.pkUserid"
      :department="user.factoryName"
    ...
    <el-container class="container bg-blue">
      <el-aside :width="side_width" class="main-aside">
        <el-tree
          node-key="to"
          :default-expanded-keys="expanded_key"
          :data="treedata"
          :indent="30"
          accordion
      ...
      <el-container class="app-main">
        <el-main>
          <span class="hide-span" @click="toggleAside">
            <i ref="hidespan" class="el-icon-arrow-left"></i>
          </span>
          <div class="admin-hl pos">
            <b>
              <i class="el-icon-arrow-down"></i>位置：
            </b>
            <el-breadcrumb separator="-">
              <el-breadcrumb-item
                :to="{name: item.to}"
                v-for="(item, i) in bread"
                :key="i"
          ...
          <div class="bg-blue pad">
            <div class="admin-search">
              <div class="options" v-if="!isProCheck">
                <el-button
                  class="admin-hl hl-btn"
                  @click="changeActive(module)"
                  type="primary"
                ...
                <el-button
                  ...
                  class="admin-hl hl-btn"
                  type="primary"
                  @click="changeActive(module, 1)"
                >{{ module.label }}列表</el-button>
              </div>
              <div class="options" v-else>
                <el-button
                  ...
                  :key="i"
                  class="admin-hl hl-btn"
                  :class="{'active-prod': item.active}"
                  type="primary"
                  @click="activateProd(item, i)"
              ...
              <el-button
                ...
                @click="gotoSlaughterList(module, 1)"
              >{{ module.label }}列表</el-button>
              <el-button
                v-if="isShop()"
                class="admin-hl hl-btn"
...
export default {
      ...
      slaughterTree: {
        ...
        children: [
          ...
          },
          {
            label: "分割管理",
            to: "segment",
            children: [{ label: "分割管理", to: "segmentManger" }]
          },
          {
            label: "溯源管理",
            to: "stock",
  ...
  mounted() {
    ...
    for (let tree of treeArr) {
      if (idx !== -1) {
        mod = tree.children.find(
          v => v.to === parent || v.to === parent + postfixArr[idx]
        );
      } else {
        mod = tree.children.find(v => v.to === parent);
      }
      if (mod && Array.isArray(mod.children)) {
        submod = mod.children.find(
          v => v.to === child + postfix || v.to === child + "prac"
        );
      } else if (mod) {
        break;
      }
    ...
    if (mod && submod) {
      ...
      arr.push(
        { text: mod.label, to: mod.to },
        { text: submod.label, to: submod.to }
      );
      this.module = submod;
    } else if (mod) {
  ...
  methods: {
    ...
    changeActive(item, isTo) {
      if (isTo) {
        ...
        if (idx === -1) {
          ...
          this.$router.push({
            name: item.to.substr(0, item.to.indexOf("prac")) + "list"
          });
        }
      } else {
    ...
    deleteItem(tree, item) {
      ...
      for (let i in tree) {
        ...
        if (tree[i].to == item) tree.splice(i, 1);
      }
    },
        ...
        if (res.data.tables.indexOf(i) == -1) {
          ...
          if (relationShip[item].path !== null) {
            for (let key in relationShip[item].name) {
              deleteTree[relationShip[item].path].push(
                relationShip[item].name[key]
              );
            }
          }
        }
