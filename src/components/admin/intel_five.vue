...
<el-card>
  <p style="color:#2891d1;font-size:15px"> 妊娠满145天羊只一览</p>
   <el-table
   @row-click="alertDetail"
    :data="tableData3"
    border
...
<script>
  import { getIntelTable , getUserById ,getStageDetail ,getStageTable ,getSheepBuilding ,getSheepCol ,moveSheep ,moveSheepAll ,updateSheepAllMe ,getFiveDetail} from '@/util/getdata'
  export default{
    props:{
      sumtext:{
        type: String
    ...
    methods:{
      ...
      fetchData(){
        ...
        getFiveDetail(this.user.userFactory).then(res=>{
